...
import com.mikm._components.routine.*;
import com.mikm.entities.actions.Action;

import java.util.*;
import java.util.function.Predicate;

public class EntityYAMLRoutineReader {
    private String fileName;
    private Map<String, EntityYAMLData.RoutineData> routineDataMap;
    private Map<String, Routine> nameToRoutine = new HashMap<>();

    private RoutineListComponent routineListComponent;

    public EntityYAMLRoutineReader(String fileName, Map<Class<? extends Component>, Component> components, Map<String, EntityYAMLData.RoutineData> routineDataMap) {
    ...
    public Map<String, Routine> loadRoutines(Map<String, String> behaviourNameToInterruptRoutine, Map<String, Action> nameToAction) {
        if (routineDataMap == null) {
            throw new RuntimeException("Couldn't load cycle data from yaml file " + fileName);
        }

        ArrayList<Routine> routines = new ArrayList<>();
        List<Transition> transitionsToInit = new ArrayList<>();

        for (Map.Entry<String, EntityYAMLData.RoutineData> routineDataEntry : routineDataMap.entrySet()) {
            String name = routineDataEntry.getKey();
            EntityYAMLData.RoutineData routineData = routineDataEntry.getValue();


            Routine routine = new Routine();
            nameToRoutine.put(name, routine);

            ArrayList<CycleStep> cycleSteps = createCycleSteps(routineData, nameToAction);
            ArrayList<Transition> transitions = createTransitions(name, routineData);
            transitionsToInit.addAll(transitions);
            routine.init(cycleSteps, transitions);
            routine.name = name;
            routines.add(routine);
        }

        routineListComponent.initRoutines(routines);

        //init ConditionTransitions
        for (Transition transition : transitionsToInit) {
    ...
    private ArrayList<CycleStep> createCycleSteps(EntityYAMLData.RoutineData routineData, Map<String, Action> nameToAction) {
    ...
    //TODO ask chatgpt to clean up
    private ArrayList<Transition> createTransitions(String routineName, EntityYAMLData.RoutineData routineData) {
        ...
        for (EntityYAMLData.RoutineData.TransitionData transitionData : routineData.TRANSITIONS) {
            if (transitionData.NO_REPEAT != null) {
                ...
                output.add(new NoRepeatTransition(noRepeatGoToString));
                ...
                output.add(conditionTransition);
            } else {
    ...
    private ConditionTransition createConditionTransition(Predicate<Entity> condition, Object listMapOrString, String routineName) {
        if (listMapOrString instanceof String) {
            ...
            Map<String, Object> weightedRandomDef = (Map<String, Object>) listMapOrString;
            ...
            LinkedHashMap<Routine, Float> weightedMap = new LinkedHashMap<>();
            float total = 0f;
            boolean foundElse = false;
            Routine elseRoutine = null;
            for (Map.Entry<String, Object> e : weightedRandomDef.entrySet()) {
            ...
            List<String> listDef = (List<String>) listMapOrString;
            ConditionTransition conditionTransition = new ConditionTransition(condition, listDef);
            List<Routine> targets = new ArrayList<>();
    ...
    private Predicate<Entity> loadCompoundCondition(EntityYAMLData.RoutineData.TransitionData transitionData) {
