...
public class ParticleEffectOLD {
    int positionOffsetRadius;
    public ParticleOLD[] particles;

    public ParticleEffectOLD(ParticleTypesOLD parameters, float x, float y) {
        createParticles(parameters, 0, x, y);
    }

    public ParticleEffectOLD(ParticleTypesOLD parameters, float angleOffset, float x, float y) {
        createParticles(parameters, angleOffset, x, y);
    }

    public void createParticles(ParticleTypesOLD parameters, float angleOffset, float x, float y) {
        int amount = RandomUtils.getInt(parameters.amountMin, parameters.amountMax);
        particles = new ParticleOLD[amount];
        positionOffsetRadius = parameters.positionOffsetRadius;

        for (int i = 0; i < amount; i++) {
            particles[i] = new ParticleOLD(parameters, parameters.usesColor ? RandomUtils.getColor(parameters.startColorMin, parameters.startColorMax) : null,
    ...
    public void spawnParticlesAt(float x, float y) {
        for (ParticleOLD particle : particles) {
